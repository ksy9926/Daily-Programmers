function solution(files) {
  files.sort((a, b) => {
    if (a.match(/\D+/)[0].toLowerCase() < b.match(/\D+/)[0].toLowerCase()) {
      return -1;
    } else if (
      a.match(/\D+/)[0].toLowerCase() > b.match(/\D+/)[0].toLowerCase()
    ) {
      return 1;
    } else {
      if (Number(a.match(/\d{1,5}/)[0]) < Number(b.match(/\d{1,5}/)[0])) {
        return -1;
      } else if (
        Number(a.match(/\d{1,5}/)[0]) < Number(b.match(/\d{1,5}/)[0])
      ) {
        return 1;
      } else 1;
    }
